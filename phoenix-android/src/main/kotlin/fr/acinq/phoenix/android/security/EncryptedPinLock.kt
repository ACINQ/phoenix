...
sealed class EncryptedPinLock: EncryptedPin.KeystoreEncrypted() {

    class SingleWallet(override val iv: ByteArray, override val ciphertext: ByteArray) : EncryptedPinLock() {
        override val name: String = "ENCRYPTED_LOCK_PIN_SINGLEWALLET"
    }

    class MultipleWallet(override val iv: ByteArray, override val ciphertext: ByteArray) : EncryptedPinLock() {
        override val name: String = "ENCRYPTED_LOCK_PIN_MULTIWALLET"
    ...
    companion object {
        ...
        fun deserialize(serialized: ByteArray): EncryptedPinLock {
            ...
            when (version) {
                SINGLE_WALLET_VERSION, MULTIPLE_WALLET_VERSION -> {
                    ...
                    return when (version) {
                        ...
                        else -> throw IllegalArgumentException("unhandled V1 lock-pin version=$version")
                    }
                }
                else -> throw IllegalArgumentException("unhandled lock-pin file version=$version")
            }
        }
    }
}
