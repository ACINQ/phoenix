 ...
 */
sealed class EncryptedSpendingPin {
     ...
     */
    data class V1(val iv: ByteArray, val ciphertext: ByteArray) : EncryptedSpendingPin() {
        override val name: String = "ENCRYPTED_SPENDING_PIN_V1"
    ...
    companion object {
        ...
        private const val FILE_NAME = "spending_pin.dat"

        /** Reads an array of byte and de-serializes it as an [EncryptedSpendingPin] object. */
        private fun deserialize(serialized: ByteArray): EncryptedSpendingPin {
        ...
        fun getSpendingPinFromDisk(context: Context): String? {
            ...
            return if (!encryptedPinFile.exists()) {
                ...
                log.warn("$FILE_NAME exists but is not usable")
                null
            } else {
        ...
        fun writeSpendingPinToDisk(context: Context, pin: String) {
            val encryptedPin = V1.encrypt(pin.encodeToByteArray())
            val datadir = getDataDir(context)
            val temp = File(datadir, "temporary_spending_pin.dat")
