...
enum AddEntryError: Error, CustomStringConvertible {
	case errorReadingSecurityFile(underlying: ReadSecurityFileError)
	...
	var description: String {
		switch self {
		case .errorReadingSecurityFile(let underlying):
			"<AddEntryError: errorReadingSecurityFile: \(underlying)>"
		case .existingSecurityFileV0:
