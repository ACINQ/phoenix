...
struct ManageBoltCard: View {
	...
	@State var cardAmounts: CardsManager.CardAmounts? = nil
	...
	func isActiveChanged() {
		log.trace("isActiveChanged()")
		
		maybeShowSaveToast()
	}
	
	func dailyLimit_currencyPickerDidChange() {
		log.trace("dailyLimit_currencyPickerDidChange()")
	...
	func monthlyLimit_currencyPickerDidChange() {
		log.trace("monthlyLimit_currencyPickerDidChange()")
		
		guard !ignoreChanges else {
			log.debug("monthlyLimit_currencyPickerDidChange(): ignoreChanges")
			return
		}
	...
	func dailyLimit_currencyConverterAmountChanged(_ result: CurrencyAmount?) {
		log.trace("dailyLimit_currencyConverterAmountChanged()")
	...
	func monthlyLimit_currencyConverterAmountChanged(_ result: CurrencyAmount?) {
		log.trace("monthlyLimit_currencyConverterAmountChanged()")
	...
	func dailyLimit_currencyChanged() {
		log.trace("dailyLimit_currencyChanged()")
	...
	func monthlyLimit_currencyChanged() {
		log.trace("monthlyLimit_currencyChanged()")
	...
	func dailyLimit_userDidEdit() {
		log.trace("dailyLimit_userDidEdit()")
	...
	func monthlyLimit_userDidEdit() {
		log.trace("monthlyLimit_userDidEdit()")
	...
	func fetchCardAmounts() async {
		...
		do {
			cardAmounts = try await Biz.business.cardsManager.fetchCardAmounts(
				payments: cardPayments,
				fetcher: nil // use default fetcher
			)
		} catch {
			log.error("CardsManager.fetchCardAmounts(): error: \(error)")
			return
		}
	}
	...
	func saveCard() {
		...
		Task {
			do {
				...
				//
				let currentCard = Biz.business.cardsManager.cardForId(cardId: cardInfo.id) ?? cardInfo
				...
				try await Biz.business.cardsManager.saveCard(card: updatedCard)
	...
	func startResetPhysicalCardProcess() {
		log.trace("startResetPhysicalCardProcess()")
