...
struct ReadCardSheet: View {
	...
	func tryMatchCard(_ queryItems: Ntag424.QueryItems) {
		...
		let cards: [BoltCardInfo] = Biz.business.cardsManager.cardsListValue
		Task {
	...
	func updateLastKnownCounter(_ matchingCard: BoltCardInfo, _ lastKnownCounter: UInt32) {
		...
		Task { @MainActor in
			do {
				let currentCard = Biz.business.cardsManager.cardForId(cardId: matchingCard.id) ?? matchingCard
				...
				log.debug("CardsManager.saveCard(): error: \(error)")
			}
		}
	}
