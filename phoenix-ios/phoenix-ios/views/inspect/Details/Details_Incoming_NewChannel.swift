...
fileprivate let filename = "Details_Incoming_NewChannel"
#if DEBUG && false
...
struct Details_Incoming_NewChannel: DetailsInfoGrid {
	...
	func general_type() -> some View {
		...
		detailsRow(
			...
			valueColumnText: "Incoming on-chain payment (new channel)"
		)
	}
	...
	var payment: Lightning_kmpNewChannelIncomingPayment {
		
		guard let required = paymentInfo.payment as? Lightning_kmpNewChannelIncomingPayment else {
