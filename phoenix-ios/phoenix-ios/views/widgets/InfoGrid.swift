...
class InfoGridViewState: ObservableObject {
...
protocol InfoGridView: View {
	...
	//
	var infoGridViewState: InfoGridViewState { get }
...
extension InfoGridView {
	...
	func rowSize(identifier: String) -> CGSize? {
		...
		return infoGridViewState.rowSizes.first { $0.identifier == identifier }?.size
	}
	
	func keyColumnSize(identifier: String) -> CGSize? {
		
		return infoGridViewState.keyColumnSizes.first { $0.identifier == identifier }?.size
	}
	...
	func keyColumnWidth(identifier: String) -> CGFloat {
		...
		let values = infoGridViewState.keyColumnSizes
