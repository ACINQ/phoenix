...
import fr.acinq.phoenix.db.CardPaymentOrderRow
...
class CardsManager(
    ...
    data class CardPayments(
        val daily: List<WalletPaymentOrderRow>,
        val monthly: List<WalletPaymentOrderRow>
    ) {
        companion object {
            fun fromMonthly(monthly: List<WalletPaymentOrderRow>, startOfDay: Long): CardPayments {
                val daily = monthly.filter { (it.completedAt ?: it.createdAt) > startOfDay }
    ...
    suspend fun fetchCardPayments(): Map<UUID, CardPayments> {
        ...
        val tempResults: MutableMap<UUID, MutableList<WalletPaymentOrderRow>> = mutableMapOf()
        fullList.forEach { row ->
            tempResults[row.cardId]?.add(row.walletPaymentOrderRow) ?: run {
                tempResults[row.cardId] = mutableListOf(row.walletPaymentOrderRow)
            }
        }
    ...
    private suspend fun fetchRecentCardPayments(minInstant: Instant): List<CardPaymentOrderRow> {
        val paymentsDb = databaseManager.paymentsDb()

        var done = false
        val maxBatchCount = 50
        var offset = 0
        var results = mutableListOf<CardPaymentOrderRow>()
        do {
            val batch = paymentsDb.listRecentCardPaymentsOrder(
                newerThanDate = minInstant.toEpochMilliseconds(),
                count = maxBatchCount,
                skip = offset
            )
    ...
    suspend fun fetchCardAmounts(
        payments: CardPayments,
        fetcher: PaymentsFetcher? = null
    ): CardAmounts {
        @Suppress("NAME_SHADOWING")
        val fetcher = fetcher ?: paymentsManager.fetcher
        ...
        payments.monthly.forEach { row ->
            fetcher.getPayment(row, WalletPaymentFetchOptions.Common)?.let { paymentInfo ->
                val info = CardAmounts.Info(
                    paymentAmount = paymentInfo.payment.amount,
