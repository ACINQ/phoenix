 ...
 */
class YadioAPI(loggerFactory: LoggerFactory) : ExchangeRateApi {
    ...
    override suspend fun fetch(targets: Set<FiatCurrency>): List<ExchangeRate> {
        val httpResponse: HttpResponse? = try {
            ExchangeRateApi.httpClient.get(urlString = "https://api.yadio.io/exrates/USD")
        } catch (e: Exception) {
        ...
        val parsedResponse: YadioResponse? = httpResponse?.let {
            try {
                ExchangeRateApi.json.decodeFromString<YadioResponse>(httpResponse.bodyAsText())
            } catch (e: Exception) {
