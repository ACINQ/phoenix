...
class CloudDataTest {
    ...
    private fun testRoundtrip(incomingPayment: IncomingPayment) {
        ...
        val (data, decoded) = CloudData.cborDeserialize(blob)
        assertNotNull(data)
        assertNotNull(decoded)
        assertIs<IncomingPayment>(decoded)
    ...
    private fun testRoundtrip(outgoingPayment: OutgoingPayment) {
        ...
        assertNotNull(data)
        assertNotNull(decoded)
        assertIs<OutgoingPayment>(decoded)
    ...
    fun read_legacy_uncompressed_pubkey() {
        ...
        val (cloudData, payment) = CloudData.cborDeserialize(Hex.decode(hexWithUncompressedKey))
        assertNotNull(cloudData)
        assertEquals(65, cloudData.outgoing?.recipient?.size)
        assertNotNull(payment)
        assertIs<OutgoingPayment>(payment)
